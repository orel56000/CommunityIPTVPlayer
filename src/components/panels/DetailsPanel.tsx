...
export const DetailsPanel = ({ item, resumeAt, episodePageUrl = null, onGoToBrowse }: DetailsPanelProps) => {
  ...
  return (
    <aside className="panel flex min-h-0 flex-1 flex-col overflow-hidden p-4 lg:p-5">
      <h3 className="shrink-0 text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Stream details</h3>
      <div className="panel-scroll mt-3 min-h-0 min-w-0 flex-1">
        {!item ? <p className="text-sm text-slate-500">Select an item to inspect metadata.</p> : null}
        {item ? (
          <div className="min-w-0 space-y-4 break-words pr-1">
            {artwork ? (
              <img
                src={artwork}
                alt={item.title}
                className="aspect-video w-full max-w-full rounded-xl border border-white/[0.06] bg-slate-950 object-cover"
                loading="lazy"
              />
            ) : null}
            ...
            <div className="min-w-0">
              ...
              <div className="mt-1 flex min-w-0 items-center gap-2">
                <code className="min-w-0 flex-1 break-all rounded-lg border border-white/[0.06] bg-slate-950/60 px-2.5 py-1.5 text-xs font-mono text-cyan-100/90">
                  {shareId}
                </code>
                <button
                  type="button"
            ...
            {episodePageUrl ? (
              <div className="min-w-0">
                ...
                <p className="mt-1 text-[11px] leading-snug text-slate-500">
                  Opens this episode after refresh or when someone opens the link. Uses your playlist name and the same
                  stream URL as anyone else sharing the link.
                </p>
                <div className="mt-2 flex min-w-0 items-center gap-2">
                  <code className="min-w-0 flex-1 break-all rounded-lg border border-white/[0.06] bg-slate-950/60 px-2.5 py-1.5 text-[11px] font-mono text-cyan-100/90">
                    {episodePageUrl}
                  </code>
                  <button
                    type="button"
